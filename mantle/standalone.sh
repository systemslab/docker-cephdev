...
# MDS #
#######
for i in `seq 0 $(($MDSs - 1))`; do
  if [ ! -e /var/lib/ceph/mds/${CLUSTER}-$i/keyring ]; then
    ...
    # bootstrap MDS
    mkdir -p /var/lib/ceph/mds/${CLUSTER}-$i
    ceph ${CEPH_OPTS} auth get-or-create mds.$i mds 'allow' osd 'allow *' mon 'allow profile mds' > /var/lib/ceph/mds/${CLUSTER}-$i/keyring
  fi

  # start MDS
  ceph-mds ${CEPH_OPTS} -i $i
done
